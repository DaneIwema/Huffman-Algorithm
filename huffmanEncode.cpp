//Nick Dunne and Dane Iwema - IT279 S2024
...
//recursive helper method for generateEncodedTable();
int genTable(Node*& root, unordered_map<char, HuffmanChar>& charTable, int code, int depth);
...
int main(int argc, char** argv)
{
    ...
    else
    {
        ...
        //TODO Output the compression % as formatted in the instructions to console.

        float compression = float(newFileLength) / float(originalCharCount) * 100;
...
int populatePriorityQueue(vector<int>& frequencyTable, priority_queue<Node*>& subtreePQ)
{
    ...
    for (int i = 0; i < 256; i++) {
        if (frequencyTable[i] > 0) {
            subtreePQ.push(new Node(char(i), frequencyTable[i]));
...
Node* generateHuffmanTree(priority_queue<Node*>& subtreePQ)
{
    ...
    while (subtreePQ.size() > 1) {
        ...
        subtreePQ.pop();
        subtreePQ.push(new Node(left, right, left->count + right->count));
    }
...
int generateEncodeTable(Node*& root, unordered_map<char, HuffmanChar>& charTable)
{
    ...
    int depth = 0;
    int code = 0;
    return genTable(root->left, charTable, code, depth + 1) + 1 + genTable(root->right, charTable, code | 1, depth + 1);
}

int genTable(Node*& root, unordered_map<char, HuffmanChar>& charTable, int code, int depth) {
    if (root->isLeaf) {
        charTable.insert(make_pair(root->data, HuffmanChar(code, depth)));
        return (depth * root->count) + 9;
    }    
    return genTable(root->left, charTable, code << 1, depth + 1) + 1 + genTable(root->right, charTable, code << 1 | 1, depth + 1);
}
...
void writeCharsToFile(unordered_map<char, HuffmanChar>& charTable, ifstream& inFile, obstream& outFile)
{
    ...
    while (inFile.get(character)) {
        outFile.Writebits(charTable.find(character)->second.code, charTable.find(character)->second.depth);
    }
    outFile.Writebits(charTable.find(char(0))->second.code, charTable.find(char(0))->second.depth);
}
