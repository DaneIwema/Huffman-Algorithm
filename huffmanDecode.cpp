//Nick Dunne and Dane Iwema - IT279 S2021
...
int main(int argc, char** argv){
    ...
    if (fileName.size() < 4 || fileName.substr(fileName.size() - 4) != ".huf"){
        ...
        //printHuffmanTreeWithCodes(huffmanTree);
...
void decodeFile(ibstream& inFile, ofstream& outFile, Node* root){
    ...
    while(!foundNullChar){
        ...
        if(curNode->isLeaf && curNode->data != 0) {
            outFile << curNode->data;
            curNode = root;
        } else if (curNode->isLeaf && curNode->data == 0)
            foundNullChar = true;
